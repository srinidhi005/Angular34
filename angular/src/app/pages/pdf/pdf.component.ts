...
var dt = new Date();
	...
	didParseCell: function (data) {
	 ...
	 willDrawCell: function (data) {
	         var rows = res.data;
		         // if (data.row.index === rows.length - 2 || data.row.index%2==0) {
		if (data.row.index ===0 || data.row.index ===2 || data.row.index ===5 || data.row.index === 8 || data.row.index ===12 || data.row.index === rows.length - 2) {
			doc.setFontStyle('bold');
						}
		if (data.row.index ===3 || data.row.index ===6 || data.row.index ===9 || data.row.index === 13 || data.row.index === rows.length - 1) {
